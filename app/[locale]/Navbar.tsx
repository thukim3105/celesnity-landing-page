...
export function Navbar() {
  ...
  const [heroStage, setHeroStage] = useState<HTMLElement | null>(null);
  ...
  useEffect(() => {
    ...
    const delay = isOverHero && !reduceMotion ? 420 : 0;
    const timer = window.setTimeout(
      () => setIsMascotDocked(!isOverHero),
      delay,
    );
  ...
  const mascotHost = isMascotDocked ? pageRoot : heroStage;
      ...
      {mascotHost &&
        createPortal(
          <div
            ...
            data-docked={isMascotDocked}
            data-leaving={isMascotDocked && isOverHero}
            ref={settingsRef}
          >
        <button
